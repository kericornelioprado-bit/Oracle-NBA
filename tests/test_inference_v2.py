...
def test_predict_today_full_flow(oracle_v2):
    ...
    oracle_v2.odds_client.get_latest_odds.return_value = [{'id': 'E1'}]
    oracle_v2.odds_client.get_player_props.return_value = {
        'bookmakers': [{
            'title': 'DK',
            'markets': [{
                'key': 'player_points',
                'outcomes': [{'name': 'LeBron James', 'description': 'Over', 'price': 1.90, 'point': 24.5}]
            }]
        }]
    }
